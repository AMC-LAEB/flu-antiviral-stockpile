...
treatment_list = ["BXM"] # "OSL", "BXM05-24", "BXM25-64", "BXM65-99"

treatment_window = [2] # 8

treatment_start_t_arr = [7] # 28, 84

baloxavir_transmission_effect_arr = [0.12, 0.45]
...
for pandemic, R in pandemic_to_R.items():
    for country in country_list:
        for baloxavir_transmission_effect in baloxavir_transmission_effect_arr:
            treatment_start_t = 7
            treatment = "BXM"
            ...
            treat_win = 2
            outdir = "./simulations/%s/%s/%s/%s_%s_AVS%03i_%s_%s_TS%03i_TW%03i_WIN%02i_BXMeff%02i"%(model, pandemic.replace("_", ""), country, country, pandemic.replace("_", ""), treatment_start_t, ttt_label, treatment, TS, TW, treat_win, baloxavir_transmission_effect*100)
            cmd = ["python", "renewal.py", "simulate", "--country", country, "--ndays", "365", "--profile", pandemic,
                   ...
                   "--outdir", outdir,
                   "--baloxavir_transmission_effect", str(baloxavir_transmission_effect),
