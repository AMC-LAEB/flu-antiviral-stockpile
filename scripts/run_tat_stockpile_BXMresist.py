...
treatment_list = ["BXM", "BXM05-24", "BXM25-64", "BXM65-99"] #["BXM10-44", "BXM10-64", ]

resistance_p_dict = {"H1RESIST":[0.094, 0.022], "H3RESIST":[0.210, 0.085]} #

treatment_window = [2, 8]

treatment_start_t_arr = [7, 28, 84] # could leave out 84
                ...
                for ttt_label, ttt_pars in time_to_treat_pars.items():
                    for treat_win in treatment_window:
                        for resist_label, resist_p_arr in resistance_p_dict.items():
                            # baloxavir resistance run
                            outdir = "./simulations/%s/%s/%s/%s_%s_AVS%03i_%s_%s_%s_TS%03i_TW%03i_WIN%02i"%(model, pandemic.replace("_", ""), country, country, pandemic.replace("_", ""), treatment_start_t, ttt_label, treatment, resist_label, TS, TW, treat_win)
