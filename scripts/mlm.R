...
m1_bf <- brmsformula(
  perc_averted_deaths_norm_z ~ 1 + avs_week + PEP + tat_delay + treat_window_extended + bxm_resist_h1 + bxm_resist_h3 + treat_bxm6599 + treat_bxm0524 + treat_bxm2564 + (1 + avs_week + PEP + tat_delay + treat_window_extended + bxm_resist_h1 + bxm_resist_h3 + treat_bxm6599 + treat_bxm0524 + treat_bxm2564|country) + (1 + avs_week + PEP + tat_delay + treat_window_extended + bxm_resist_h1 + bxm_resist_h3 + treat_bxm6599 + treat_bxm0524 + treat_bxm2564|pandemic),
  family = gaussian()
)
...
m1 <- brm(
  ...
  chains = 4,
  cores = 4, 
  file = "./manuscript/m1extz_rerun27jan.rds",
  silent = 1,
  backend = "cmdstanr", 
  threads = threading(2)
)
...
write.csv(fe, file = "./manuscript/m1_fixed_effects.csv")

re_pandemic <- m1 %>% 
  spread_draws(r_pandemic[condition,term])
write.csv(re_pandemic, file = "./manuscript/m1_re_pandemic.csv")

re_country <- m1 %>% 
  spread_draws(r_country[condition,term])
write.csv(re_country, file = "./manuscript/m1_re_country.csv")
